...
let greenIcon = new L.Icon({
    ...
    let redIcon = new L.Icon({
        ...
        for(let { position:{lat,lng},...station} of stations){
        L.marker([lat, lng],{icon: greenIcon}).on("click", () =>this.onClick(station)).addTo(map)
        ...
        L.marker([lat, lng],{icon: orangeIcon}).on("click", () =>this.onClick(station)).addTo(map)      
        };
}
...
class MapController {
        constructor(whenStationSelected = ()=> undefined){
                ...
                this.stations = []
                this.view = new Mapview((station) => whenStationSelected(station))
...
class InfoStationView {
        render(station) {
                ...
                document.getElementById('number_bike').textContent = `Nombre de vélos disponible : ${station.available_bikes}`
...
const form = new FormulaireController(({firstName, lastName}) => {
